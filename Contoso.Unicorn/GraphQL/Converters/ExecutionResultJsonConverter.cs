...
namespace Contoso.Unicorn.GraphQL.Converters
{
    /// <inheritdoc />
    public class ExecutionResultJsonConverter : JsonConverter<global::GraphQL.ExecutionResult>
    {
        ...
        /// <inheritdoc/>
        public override void Write(Utf8JsonWriter writer, global::GraphQL.ExecutionResult value, JsonSerializerOptions options)
        {
            if (writer == null) throw new ArgumentNullException(nameof(writer));

            if (value == null) throw new ArgumentNullException(nameof(writer));

            writer.WriteStartObject();

            if (value is ExecutionResult executionResult)
            {
                WriteProperty(writer, "statusCode", (int)executionResult.StatusCode, options);
                WriteProperty(writer, "message", executionResult.Message, options);
                if (!string.IsNullOrEmpty(executionResult.Error))
                    WriteProperty(writer, "error", executionResult.Error, options);
                if (executionResult.Elapsed != null)
                    WriteProperty(writer, "elapsed", executionResult.Elapsed.ToString(), options);
            }
        ...
        private static void WriteData(Utf8JsonWriter writer, global::GraphQL.ExecutionResult result, JsonSerializerOptions options)
        {
            if (result.Executed)
            {
        ...
        private static void WriteExtensions(Utf8JsonWriter writer, global::GraphQL.ExecutionResult result, JsonSerializerOptions options)
        {
        ...
        /// <inheritdoc/>
        public override global::GraphQL.ExecutionResult Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        ...
        public override bool CanConvert(Type typeToConvert)
        {
            return typeToConvert == typeof(ExecutionResult) ||
                   typeToConvert == typeof(global::GraphQL.ExecutionResult);
        }
    }
}